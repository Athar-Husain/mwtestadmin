...
// import TaskDetailDialog from './TaskDetailDialog'; // Updated import
import TaskDetailDialog from './TaskDetails/TaskDetailDialog'; // Updated import
...
export const initialTasks = [
    {
        ...
        assignedTo: 'user-1',
        dueDate: '2025-07-10',
        createdAt: '2025-05-28T12:00:00Z', // Added creation date
        updatedAt: '2025-05-28T12:05:00Z', // Added updated date
    },
    {
        id: 'task-102',
        projectId: 'project-1',
...
const Index = () => {
    ...
    const handleAddTask = (newTaskData) => {
        const now = dayjs().toISOString(); // Current time in ISO format
        const newTaskId = `task-${Date.now()}`;
        const newTask = { id: newTaskId, ...newTaskData, createdAt: now, updatedAt: now };
    ...
    const handleUpdateTask = (updatedTask) => {
        setTasks(prevTasks => prevTasks.map(task =>
            task.id === updatedTask.id ? { ...updatedTask, updatedAt: dayjs().toISOString() } : task // Update timestamp
        ));
        // No need to close dialog here, TaskDetailDialog manages its own internal editing state
        // and calls onClose when it's completely done.
    };
    ...
    const onDragEnd = (result) => {
        ...
        if (isStatusBoard) {
            ...
            const updatedTask = { ...draggedTask, status: newStatus, updatedAt: dayjs().toISOString() };
            newTasks.push(updatedTask);
            setTasks(newTasks);
        } else {
            const newAssignedTo = destination.droppableId;
            const updatedTask = { ...draggedTask, assignedTo: newAssignedTo, updatedAt: dayjs().toISOString() };
            newTasks.push(updatedTask);
            setTasks(newTasks);
        }
    };
